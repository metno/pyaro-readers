...
class EbasColDef(dict):
    ...
    def get_wavelength_nm(self):
        ...
        if not "wavelength" in self:
            raise KeyError(
                f"Column variable {self.name} does not contain wavelength information"
            )
...
def _readline_ref_and_revision(line):
    ...
    basedate = np.datetime64(
        datetime.strptime(f"{spl[0]}{spl[1]}{spl[2]}", "%Y%m%d"), "s"
    )
...
class NasaAmesHeader:
    ...
    _H_FIXLINES_YIELD = [
        ["num_head_lines", "num_head_fmt"],  # line 1
            ...
            for flag in not_ok:
                ...
                for val in vals:
                    if val == 100:
                        ...
                        val in self.FLAG_INFO["valid"]
                        and not self.FLAG_INFO["valid"][val]
                    ):  # is invalid
                        _invalid = True
...
class EbasNasaAmesFile(NasaAmesHeader):
    ...
    **kwargs
    ...
    def read_file(
        ...
        for line in lines:
            if in_data:  # in data block (end of file)
                ...
                lc < self._NUM_FIXLINES
            ):  # in header section (before column definitions)
                try:
                    ...
                    msg = (
                        f"Failed to read header row {lc}.\n{line}\nError msg: {repr(e)}"
                    )
    ...
    def _read_vardef_line(self, line_from_file):
        ...
        spl = [
            x.replace(",", ", ") for x in spl
        ]  # add space after comma again in quoted fields
        name = spl[0]
        if not len(spl) > 1:
            unit = ""
        else:
